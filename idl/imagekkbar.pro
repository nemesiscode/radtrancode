pro imagekkbar,kkin,tname,iplotx,nplotx,iploty,nploty,$
...
if(iplotx eq 1 and iploty eq 1) then begin
  plot_io,x,y,xstyle=1,ystyle=1,yrange=[pmax,pmin],$ 
  title=tname,xtitle=xname,$
  ytitle='Pressure (bar)',thick=lx,xthick=lx,ythick=lx,charthick=lx,charsize=ly
endif else begin
  plot_io,x,y,/noerase,/nodata,yrange=[pmax,pmin],$
...
!p.position=[x0+lm,y0+bm,x1-rm,y1-tm]
plot_io,x,y,/noerase,xticklen=1.0,yrange=[pmax,pmin],$
