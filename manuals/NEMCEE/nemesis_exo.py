...
# a 0th-size array
    ...
    model = nemesisdiscEMCEE.nemesisdiscemcee(runname, len(y), len(tcubic), len(vmr), ith, tcubic, vmr, vflag, gflag, mass, rad)
#    model = 1
...
# maximum likelihood params
 ...
 print(runname)
 medmodel = nemesisdiscEMCEE.nemesisdiscemcee(runname, len(y), len(tempmed), len(vmrmed), 0, tempmed, vmrmed, vflag, gflag, massmed, radmed)
 highmodel = nemesisdiscEMCEE.nemesisdiscemcee(runname, len(y), len(temphigh), len(vmrhigh), 1, temphigh, vmrhigh, vflag, gflag, masshigh, radhigh)
 lowmodel = nemesisdiscEMCEE.nemesisdiscemcee(runname, len(y), len(templow), len(vmrlow), 2, templow, vmrlow, vflag, gflag, masslow, radlow)
