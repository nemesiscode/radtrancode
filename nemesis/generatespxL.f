...
C     ******************************************************************
C
C     CIRS retrieval code utilising correlated-k or lbl tables,
C     fast gradient radiative transfer model CIRSRADG. Extension of
C     NemesisL to retrieve from a number of locations in a row.  
C
C     NemesisL is not set up for dealing with scattering calculations yet
C
...
C     TIME2: System time at the end of program execution.
      ...
      integer lx(mx),iprfcheck,ifix(mx),occult,ilbl,inum,ishape,ilbl1
...
C     Add forward errors to measurement covariances
     ...
     3     nx,xn,ny,yn,kk)      
       ...
       print*,'Scattering calculations not allowed in GeneratespxL'
       stop
      endif
