      subroutine gsetradV(runname,iscat,nmu,mu,wtmu,isol,dist,
...
C     ************************************************************************
      ...
      if(idiag.gt.0)print*,'gsetradV, lin = ',lin
...
51       read(9,1)buffer
         ...
         if(ncont1.ne.ncont)then
          if(idiag.gt.0)print*,'Error in gsetradV ncont1 <> ncont'
...
54       read(9,1)buffer
         ...
         if(nalb1.ne.nalb)then
          if(idiag.gt.0)print*,'Error in gsetradV nalbx <> nalb1'
...
C       ***************** Surface temperature correction ***********
      ...
      do ivar=1,nvar
       ...
       if(idiag.gt.0)print*,'gsetradV - ',ivar,varident(ivar,1)
...
55      read(9,1)buffer
        ...
        read(buffer,*)nalb1
        if(nalb1.ne.nalb)then
         if(idiag.gt.0)print*,'Error in gsetradV nalb <> nalb1'
         if(idiag.gt.0)print*,nalb,nalb1
         if(idiag.gt.0)print*,'file : ',runname
        endif
...
C     See if Sromovsky cloud layer model is specified.
      if(idiag.gt.0)then
       print*,'gsetradV - iflagcloud,iflagsrom = ',iflagcloud,iflagsrom
       print*,'gsetradV - cbflag, iflagcb = ',cbflag,iflagcb
      endif

      if(iflagcloud)then

        print*,'Models not specified in gsetradV.f'
        stop

      else

       call gwritepatV(runname,gasgiant,iscat,sol_ang,
...
105      CONTINUE
     ...
     1 or.varident(ivar,3).eq.8.or.varident(ivar,3).eq.32)then
          ...
          if(varident(ivar,3).eq.14.or.varident(ivar,3).eq.15)then
              ...
              xscal(icont)=xod(icont)/od1
              if(idiag.gt.0)then
               print*,'gsetradV - icont,od1,xscal(icont) = ',icont,
     1          od1,xscal(icont)
              endif
              do j=1,NN
               dust(icont,j)=dust(icont,j)/xscal(icont)
...
C          np1 should now match nwave
           if(np1.ne.nwave)then
             if(idiag.gt.0)then
             print*,'Warning from gsetradV.f, nwave in refindex file is'
