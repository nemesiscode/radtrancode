      SUBROUTINE GWRITEPATMCS(RUNNAME,ISCAT,NCONV,VCONV,FWHM,
     1 LAYHT,TOPHT,NLAYER,LAYTYP,LAYINT,FLAGH2P)
C     $Id:
C     *******************************************************************
C     Subroutine to write out the .pat file needed for a CIRSradg (MCS
C     variant) run.
C 
C     Input variables:
...
C	TOPHT		real		Height of top of atm.
...
C     *******************************************************************
      ...
      REAL TOPHT,DELH,RADIUS,XH,THETA
      INTEGER ILAYER
      INTEGER NBASE,IBASE
...
101   CONTINUE
      ...
      NBASE = INT(0.5*NLAYER)
      RADIUS=3397.0

      DO 201 IBASE=1,NBASE+1
       XH = (IBASE-1)*DELH
       THETA = (180./3.1415927)*ASIN((RADIUS-XH)/RADIUS)

C      Atmospheric thermal emission
       WRITE(31,1)'atm'
       TEXT='nadir'
       LAYBOT=1
       WRITE(31,4)TEXT,THETA,LAYBOT
4      FORMAT(A6,F7.2,I4)
       IF(ISCAT.EQ.0)THEN
        WRITE(31,1)'therm'
        WRITE(31,1)'noscatter'
