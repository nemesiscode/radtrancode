...
C     ****************************************************************
      ...
      real efsh,xfsh,varparam(mvar,mparam),gasdev
...
C           Now smooth with covariance matrix
            do i=1,np
             xn(ix+i)=0
             do j=1,np
        xn(ix+i)=xn(ix+i) + (sx(ix+i,ix+j)/sx(ix+i,ix+i))*xtry(j)**2
             enddo
             xn(ix+i)=xn(ix+i)*sqrt(xn(ix+i))
            enddo          
...
C         ******** profile held as deep amount, fsh and knee pressure **
            np = 2
            do i=1,np
             xn(ix+i)=x0(ix+i)+err(ix+i)*gasdev(idum)
            enddo
