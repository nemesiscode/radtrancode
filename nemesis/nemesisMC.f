...
C     Make sure input files are OK
      ...
      if(npro.gt.maxpro)then
       print*,'Error in NemesisMC. npro > maxpro : ',npro,maxpro
       stop
      endif
...
C      open previous raw retrieval file (copied to .pre)
       ...
       print*,'NemesisMC: reading previous retrieval : ',runname
...
C      print*,'JSURF: ',jsurf,xa(jsurf)
      ...
      call coreretMC(runname,ispace,iscat,ica,kiter,phlimit,
     1  fwhm,xlat,ngeom,nav,nwave,vwave,nconv,vconv,angles,
     2  gasgiant,lin,lpre,nvar,varident,varparam,npro,jsurf,jalb,jtan,
     3  jpre,jrad,wgeom,flat,nx,xa,sa,ny,y,se,xn,sm,sn,st,yn,kk,aa,dd)
...
C     write output
      print*,'NemesisMC: Writing output'
