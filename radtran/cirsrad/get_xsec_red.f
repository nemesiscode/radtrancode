...
C-----------------------------------------------------------------------
	...
	call read_xsec(xscfil)
        ...
        if(idiag.gt.0)print*,'get_xsec_red: read_xsec OK'
	ncont1 = ncont
	nsec1 = nsec
	...
	do I = 1, nsec
		...
		do J = 1, ncont
			call read_hg(vwave,J,ncont,f,g1,g2)
                        if(idiag.gt.0)print*,'get_xsec_red ',f,g1,g2
			asym = f*g1+(1-f)*g2
			if(idiag.gt.0)print*,'get_xsec_red ',f, g1, g2
     			if(idiag.gt.0)print*,'get_xsec_red ',asym
			if (asym.GE.0) then
			  xsec1(1,J,I) 
     1	                  = xsec(1,J,I)-real(asym)*xsec(2,J,I)
			...
			if(idiag.gt.0)print*,'get_xsec_red xsec1, xsec'
                        if(idiag.gt.0)print*,xsec1(1,J,I),xsec(1,J,I)
		enddo
	enddo
