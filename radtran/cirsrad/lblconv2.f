...
C       Updated version of lblconv1.f to use trapezium rule integration
C
C-----------------------------------------------------------------------

	SUBROUTINE lblconv2(runname,fwhm, ishape,nwave, vwave, y, 
...
C       bins, paths, etc.)
	...
	REAL		vwave(nwave), y(maxout), vconv(nconv),
     1			yout(maxout), ynor(maxout), f1,f2
...
C           Triangular Instrument Shape
            ...
            F2=1.0 - ABS(VWAVE(I+1)-VCEN)/YFWHM
           ELSEIF(ISHAPE.EQ.2)THEN
C           Gaussian Instrument Shape
            F1=EXP(-((VWAVE(I)-VCEN)/SIG)**2)
            F2=EXP(-((VWAVE(I+1)-VCEN)/SIG)**2)
...
C           Hanning Instrument Shape
           ...
           IF(F1.GT.0.0.OR.F2.GT.0.0)THEN
             YOUT(J)=YOUT(J)+0.5*(F1*Y(I)+F2*Y(I+1))
             YNOR(J)=YNOR(J)+0.5*(F1+F2)
           ENDIF
...
C         Channel Integrator Mode: Slightly more advanced than previous
          ...
          OPEN(17,FILE=runname,STATUS='old')
          READ(17,*)NCONV1
          DO 200 K=1,nconv1
           READ(17,*)vcentral
           READ(17,*)nsub
           do j=1,nsub
            read(17,*)vfil(j),fil(j)
           enddo
...
C            Find relevant points in tabulated files.
             ...
             DO 202 I=I1,I2-1
              ...
              CALL interp(vfil,fil,nsub,f2,vwave(I+1))
