      SUBROUTINE DLUDCMP(A,NDIM,MDIM,INDX,D)
C***********************************************************************
C_TITL:	DLUDCMP.f
C
...
C TINY: A small number.
      ...
      INTEGER I,J,K,IMAX,NDIM,MDIM,INDX(MDIM)
...
C******************************** CODE *********************************
      ...
      IF(NDIM.GT.MDIM) THEN 
       PRINT*,'Error in DLUDCMP.f: NDIM > MDIM',NDIM,MDIM
       STOP
      ENDIF

      IF(MDIM.GT.IDIM) THEN 
       PRINT*,'Error in DLUDCMP.f: MDIM > IDIM',MDIM,IDIM
       STOP
      ENDIF
...
11      CONTINUE
        IF(AAMAX.EQ.0.)THEN
          WRITE(*,*)'DLUDCMP.f :: *ERROR* Singular matrix. Stopping'
          WRITE(*,*)'program.'
          WRITE(*,*)' '
