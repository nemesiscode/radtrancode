...
C     DUDS = DU/DS = the number of molecules per cm2 per km along the path
      ...
      REAL CLBOTH(10),CLTOPH(10),COD(10),CWID
      INTEGER NCLAY(10)
...
C        computing the cloud heights of each layer
	 ...
	 FSH(J)=D1
157     CONTINUE
        READ(2,*,END=3)KEXT,CWID
        print*,'NCOMPLAYERB: CWID = ',CWID
...
C     Now need to force the extended cloud opacity to drop to zero near
C     the tropopause.
      XFAC=1.
      DO I=1,NLAY
       IF(BASEP(I).GT.0.1)THEN
        XFAC = 1.0 - EXP(-((LOG(BASEP(I))-LOG(0.1))/CWID)**2)
       ELSE
        XFAC=0.
       ENDIF
       CONT(KEXT,I)=CONT(KEXT,I)*XFAC
       print*,I,BASEP(I),KEXT,CONT(KEXT,I),XFAC
      ENDDO

C     Now normalised the cloud opacity
      DO J=1,NCLOUD
       CDUM(J)=0.0
      ENDDO
      DO I=1,NLAY
       ...
       ENDDO
      ENDDO
      PRINT*,'Unmormalised Cloud ODs : ',(CDUM(J),J=1,NCLOUD)
      
      DO I=1,NLAY
       DO J=1,NCLOUD
        CONT(J,I)=COD(J)*CONT(J,I)/CDUM(J)
       ENDDO
      ENDDO
