...
c	print*,10,tstim_arr(line),absco_arr(line),ad_arr(line)
101     CONTINUE
...
C        Figure out which bins are needed here
         ...
         print*,FSTBIN,V1,LSTBIN,V2,IBINMAX

         IF(LSTBIN.GT.IBINMAX)THEN
C         Need to read in new lines
          ...
          NXTREC= DBREC
          FIRST=1 
          print*,'NXTREC,VFIRST,RANGE1',NXTREC,VFIRST,RANGE1     
...
C          Last bin which is definitely full
          ...
          DO IBIN=1,NBIN 
            IF(NLINES(IBIN).GT.0)THEN 
             PRINT*,IBIN,VBIN(IBIN),NLINES(IBIN),FSTLIN(IBIN),
     1        LSTLIN(IBIN)
            ENDIF
          ENDDO
...
c	print*, 11,absco_arr(line),tstim_arr(line),ad_arr(line),y_arr(line)
