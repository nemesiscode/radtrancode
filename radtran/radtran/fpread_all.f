      subroutine fpread_all(vv,temp,kpara,absh2,abshe,dah2dT,dahedT,
     1 deltaf,abs,dabsdT)
C     **********************************************************************
...
C     Fletcher (22/06/17) Updated to read in additional gases as well
...
C     Other variables
      ...
      real u,t,temp1,dudt
      integer cv,ct,pindex,i,j,k,ii,jj,kk
      integer kpara
      logical fexist
...
C     define the fixed parameters
      ...
      if(iread2.eq.1)then
        ...
        if(fexist)then
         if(idiag.gt.0)print*,'fpread_all. Reading: ',aname2
        else
         if(idiag.gt.0)then
          print*,'fpread_all. File does not exist : ',aname2
         endif
        endif
    
        open(12,file=aname2,form='unformatted',status='old')      
         if(idiag.gt.0)print*,'FPREAD_ALL. Opening look-up file'
...
C       5............H2-CH4
	 ...
	 if(idiag.gt.0)print*,'Reading additional gases:'
	 read(12)kcia5
...
C     initialise the absorption array to be zero in case of failure
      do 34 i=1, NUMPAIRS-4
            abs(i) = 0.0
            dabsdT(i)=0.0
34    continue
 

         if(idiag.gt.0)print*,'fpread_all : New Data read in'
...
C     ----------------------------------------------------------------------
      ...
      if(vv.gt.vvk2(NUMWN).or.vv.lt.vvk2(1).and.ivk.ne.1)then
         if(idiag.gt.0)print*,'fpread_all : wavenumber out of range'
         ivk=1
         return
      end if

      if(temp.gt.tempk2(NUMT).and.it1k.ne.1)then
         if(idiag.gt.0)print*,'fpread_all: warning temp > MaxT '
      ...
      if(temp.lt.tempk2(1).and.it2k.ne.1)then
         if(idiag.gt.0)print*,'fpread_all: warning temp <  MinT ',temp
...
C     are the same, of course, for both H2-H2 and H2-He gas pairs, and 
C     for all the NUMPAIRS gas pairs.
...
26    continue
      ...
      do 27 i=1, NUMPAIRS-4
         abs11 = kcia5(i,ct,cv)
         abs12 = kcia5(i,ct,cv+1)
         abs22 = kcia5(i,ct+1,cv+1)
         abs21 = kcia5(i,ct+1,cv)
