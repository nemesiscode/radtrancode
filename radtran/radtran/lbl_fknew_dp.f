...
c LNF:  Changed output to DP
      DOUBLE PRECISION OUTPUT(MPOINT)
      DOUBLE PRECISION TX(MPOINT)
      CHARACTER*1 ANS
...
c LNF:  Changed VMIN et al. to DP.

      DOUBLE PRECISION VMIN,DELV,WING,DV
...
c LNF removed single precision:        V = SNGL(VV)
	V = VV
...
52      CONTINUE
        IF(TAUTMP.LT.0.0)THEN
 	   WRITE(*,*)' LBL_FKNEW :: *ERROR* TAUTMP < 0.0!'
