      real function tgv3(imod,qrot,tpart,knu0,delad,y0,a,E1,E2,
     1  SFB,P,T,U,q)
C     $Id:
C     *********************************************************************
C
...
C     Input Variables
C
C	tpart(4) real	Partition function coeffs.
...
C	E1	real	Lower state energy 1 (cm-1)
C	E2	real	Lower state energy 2 (cm-1)
...
C     Output variable
C
C 	tgv3	real	Transmission of path
C     ------------------------------------------------------------------------
C
C	Pat Irwin	26/9/94
C
C     *********************************************************************
      implicit none
      real knu,U,delad,y,T,E1,E2,knu0,C1,T0,y0,SFB,P,q,P0,qrot,a
...
C      print*,'tgv3'
C      print*,tpart
C      print*,knu0,delad,y0,E1,E2,SFB
C      print*,P,T,U,q
...
C     Calculate absorption coefficient at path temperature
      ...
      if(imod.eq.2)then
       xpart0 = tpart(1)+T0*(tpart(2)+T0*(tpart(3)+T0*tpart(4)))
       xpart = tpart(1)+T*(tpart(2)+T*(tpart(3)+T*tpart(4)))
      else
       xpart0 = T0**1.5
       xpart = T**1.5
      endif

      knu1=knu0*(xpart0/xpart)*exp(C1*E1*(1/T0 - 1/T))
      knu2=knu0*(xpart0/xpart)*exp(C1*E2*(1/T0 - 1/T))
      
      knu = a*knu1 + (1.0-a)*knu2
...
C      print*,knu0,knu
      ...
      if(lx.eq.0)then
       print*,'tgv3: Error: lx = 0.'
       print*,'knu0,delad,y0,E1,E2,SFB,P,T,U,q',knu0,delad,
     1		y0,E1,E2,SFB,P,T,U,q
