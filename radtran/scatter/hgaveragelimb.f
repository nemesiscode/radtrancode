      subroutine hgaveragelimb(nlays,ncont,taus,eps,lfrac,tauray,
     1   vwave,f1,g11,g21)
C     ***************************************************************
C     Subroutine to find the average H-G phase function in each layer 
C     along a path in an atmosphere by adding up the OD-weighted HG phase functions
C     of constituent aerosols, and also Rayleigh scattering if required.
C
C     Input variables
C       nlays	integer	Number of atmospheric layers in path
C	ncont	integer	Number if aerosol types
C	taus(maxlay) real optical depth of each layer
C	eps(maxlay) double fraction of thermal emission in each layer
C	lfrac(maxcon,maxlay) real fraction of scattering for each 
C				   particle type in each layer
C	tayray(maxlay) real optical depth in each layer due to Rayleigh
C			      scattering
C	vwave real wavenumber/wavelength
C
C     Output variables
C	f1(maxlay)  real averaged HG f value
C	g11(maxlay) real averaged HG g1 value
C	g12(maxlay) real averaged HG g2 value
C
...
C     ***************************************************************
      ...
      real f1(maxlay),g11(maxlay),g21(maxlay),
     1 taus(maxlay)
      real tauray(maxlay),lfrac(maxcon,maxlay),vwave
      real tf(maxcon),tg1(maxcon),tg2(maxcon)
      double precision f,g1,g2,eps(maxlay),omega
